...
但是由于积分误差的存在，显然会随着行驶不断累计，直到严重偏离而令人无法接受

但是，只需要一块用于探测当前状态下周围是否存在地标的红外传感器，在合适的情况下，就可以完全消除累计误差。

当红外传感器检测到存在的地标时，我们通过编码已经拥有了一个不那么可靠的位置和方向数据，
...
或者使用包括但不限于空间划分树、平面四叉树，层次包围盒树等数据结构对我们需要查找最邻近地标的地图进行划分，或者使用带有地标ID的距离场来加速就临近的过程。  
